...
initView = data => {
    ...
    switch (mode) {
        ...
        case Constants.Mode.GROUP:
            ...
            JSON.parse(data.groupIds).forEach((id, i) => { urls[i] = hostname + '/sections?groupId=' + id; });
            break;
        case Constants.Mode.SECTION:
            ...
            JSON.parse(data.sectionIds).forEach((id, i) => { urls[i] = hostname + '/sections/' + id; });
            break;
        default:
    ...
    urls.forEach(async url => {
        ...
        if (count === 0 && Object.keys(sections).length > 0) {
            ...
            if (Object.keys(sections).length > 1) {
                let max = 0;
                Object.keys(sections).forEach(space => {
            ...
            if (sectionsToReplicate.length > 0) {
                await replicate(sectionsToReplicate, {
...
replicate = async (sections, config) => {
    const minMax = { x: { min: Number.MAX_VALUE, max: Number.MIN_VALUE }, y: { min: Number.MAX_VALUE, max: Number.MIN_VALUE } };
    ...
    if (viewport.w > 0 && viewport.h > 0) {
        ...
        const clients = JSON.parse(await (await fetch(config.hostname + '/spaces')).text())[config.space];
        clients.forEach((c, i) => {
            if (!(c.x !== undefined && c.y !== undefined && c.w !== undefined && c.h !== undefined &&
                ((+c.x + c.w) > viewport.x && +c.x < (viewport.x + viewport.w)) &&
                ((+c.y + c.h) > viewport.y && +c.y < (viewport.y + viewport.h)))) return;
            $('<iframe>', {
        ...
        setTimeout(() => {
            const filter = [];
            sections.forEach(section => filter.push(section.id));
