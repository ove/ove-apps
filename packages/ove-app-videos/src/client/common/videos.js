...
initCommon = () => {
    ...
    const correctPosition = () => {
        try {
            ...
            Object.values(context.sync.clients).forEach(peer => {
    ...
    window.ove.socket.addEventListener(async message => {
        ...
        if (message.state) {
            ...
            await handleStateChange(message.state);
            return;
...
handleStateChange = async state => {
    ...
    if (current.url !== state.url) {
        ...
        if (!stateURL.hostname.includes('youtube')) {
            ...
            log.error(`Youtube URL ${state.url} is not in format https://www.youtube.com/embed/<VIDEO_ID>`);
            return;
        }

        await context.player.initialize();
...
handleBufferStatusChange = function (status) {
    ...
    } else if (!status.type.update || !context.bufferStatus.clients.includes(status.clientId)) return;
    ...
    if (status.percentage < Constants.MIN_BUFFERED_PERCENTAGE &&
        status.duration < Constants.MIN_BUFFERED_DURATION) return;
